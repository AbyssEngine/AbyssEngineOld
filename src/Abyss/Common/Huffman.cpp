#include "Huffman.h"

#include <array>

namespace Abyss::Common {

static constexpr int decompVal1 = 256;
static constexpr int decompVal2 = 257;
static constexpr std::array<std::array<uint8_t, 256>, 9> _primes = {
    {{{
         // Compression type 0
         ,
     }},
     {{
         // nolint:dupl // it doesnt matter here
         //  Compression type 1
         ,
     }},
     {{
         // Compression type 2 //nolint:dupl // it doesnt matter here
         0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x03, 0x27, 0x00, 0x00, 0x23, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
         0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x02, 0x02, 0x01, 0x01,
         0x06, 0x0E, 0x10, 0x04, 0x06, 0x08, 0x05, 0x04, 0x04, 0x03, 0x03, 0x02, 0x02, 0x03, 0x03, 0x01, 0x01, 0x02, 0x01, 0x01, 0x01, 0x04,
         0x02, 0x04, 0x02, 0x02, 0x02, 0x01, 0x01, 0x04, 0x01, 0x01, 0x02, 0x03, 0x03, 0x02, 0x03, 0x01, 0x03, 0x06, 0x04, 0x01, 0x01, 0x01,
         0x01, 0x01, 0x01, 0x02, 0x01, 0x02, 0x01, 0x01, 0x01, 0x29, 0x07, 0x16, 0x12, 0x40, 0x0A, 0x0A, 0x11, 0x25, 0x01, 0x03, 0x17, 0x10,
         0x26, 0x2A, 0x10, 0x01, 0x23, 0x23, 0x2F, 0x10, 0x06, 0x07, 0x02, 0x09, 0x01, 0x01, 0x01, 0x01, 0x01,
     }},
     {{
         // nolint:dupl // it doesnt matter here
         //  Compression type 3 //nolint:dupl // it doesnt matter here
         ,
     }},
     {{
         // Compression type 4 //nolint:dupl // it doesnt matter here
         0xFF,
         0xFB,
         0x98,
         0x9A,
         0x84,
         0x85,
         0x63,
         0x64,
         0x3E,
         0x3E,
         0x22,
         0x22,
         0x13,
         0x13,
         0x18,
         0x17,
     }},
     {{
         // Compression type 5 //nolint:dupl // it doesnt matter here
         0xFF, 0xF1, 0x9D, 0x9E, 0x9A, 0x9B, 0x9A, 0x97, 0x93, 0x93, 0x8C, 0x8E, 0x86, 0x88, 0x80, 0x82, 0x7C, 0x7C, 0x72, 0x73, 0x69, 0x6B,
         0x5F, 0x60, 0x55, 0x56, 0x4A, 0x4B, 0x40, 0x41, 0x37, 0x37, 0x2F, 0x2F, 0x27, 0x27, 0x21, 0x21, 0x1B, 0x1C, 0x17, 0x17, 0x13, 0x13,
         0x10, 0x10, 0x0D, 0x0D, 0x0B, 0x0B, 0x09, 0x09, 0x08, 0x08, 0x07, 0x07, 0x06, 0x05, 0x05, 0x04, 0x04, 0x04, 0x19, 0x18,
     }},
     {{
         // nolint:dupl // it doesnt matter here
         //  Compression type 6
         ,
     }},
     {{
         // nolint:dupl // it doesnt matter here
         //  Compression type 7
         ,
     }},
     {{
         // Compression type 8
         ,
     }}}};

Huffman::LinkedNode::LinkedNode(int decompressedValue, int weight)
    : _decompressedValue(decompressedValue), _weight(weight), _firstChild(), _prevSibling(), _nextSibling() {}

auto Huffman::LinkedNode::getDecompressedValue() const -> int { return _decompressedValue; }

auto Huffman::LinkedNode::getWeight() const -> int { return _weight; }

auto Huffman::LinkedNode::getParent() const -> std::shared_ptr<LinkedNode> { return _parent; }

auto Huffman::LinkedNode::getFirstChild() -> std::shared_ptr<LinkedNode> { return _firstChild; }

auto Huffman::LinkedNode::getPrevSibling() const -> std::shared_ptr<LinkedNode> { return _prevSibling; }

auto Huffman::LinkedNode::getNextSibling() const -> std::shared_ptr<LinkedNode> { return _nextSibling; }

auto Huffman::LinkedNode::getChild1() const -> std::shared_ptr<LinkedNode> { return _firstChild ? _firstChild->getPrevSibling() : nullptr; }

auto Huffman::LinkedNode::setNextSibling(const std::shared_ptr<LinkedNode> &nextSibling) -> void { _nextSibling = nextSibling; }

auto Huffman::LinkedNode::setPrevSibling(const std::shared_ptr<LinkedNode> &prevSibling) -> void { _prevSibling = prevSibling; }

auto Huffman::LinkedNode::insert(std::shared_ptr<LinkedNode> other) -> std::shared_ptr<LinkedNode> {
    if (other->getWeight() <= _weight) {
        if (_nextSibling != nullptr) {
            _nextSibling->setPrevSibling(other);
            other->setNextSibling(_nextSibling);
        }

        _nextSibling = other;
        other->setPrevSibling(shared_from_this());

        return other;
    }

    if (_prevSibling == nullptr) {
        other->setPrevSibling(nullptr);
        _prevSibling = other;
        other->setNextSibling(shared_from_this());
    } else {
        _prevSibling->insert(other);
    }

    return shared_from_this();
}

// func decode(input *d2datautils.BitStream, head *linkedNode) *linkedNode {
// node := head
//
//	for node.child0 != nil {
//     bit := input.ReadBits(1)
//               if bit == -1 {
//             log.Fatal("unexpected end of file")
//         }
//
//         if bit == 0 {
//                    node = node.child0
//                           continue
//                }
//
//                 node = node.getChild1()
//     }
//
//     return node
// }

} // namespace Abyss::Common
