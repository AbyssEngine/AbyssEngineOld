...
Crypto::Crypto() : cryptTable({}) { initializeCryptTable(); }

auto Crypto::getInstance() -> Crypto & {
    static Crypto instance;
    return instance;
}

auto Crypto::hashString(const std::string_view key, const uint32_t hashType) const -> uint32_t {
...
auto Crypto::hashFileName(const std::string_view fileName) const -> uint64_t {
...
auto Crypto::initializeCryptTable() -> void {
...
auto Crypto::decrypt(std::istream &input, std::ostream &output, uint32_t seed) const -> void {
    uint32_t seed2 = 0xEEEEEEEE;
    uint32_t value = 0;
...
auto Crypto::decrypt(std::vector<uint32_t> &input, uint32_t seed) const -> void {
    uint32_t seed2 = 0xEEEEEEEE;
...
auto Crypto::decrypt(std::vector<std::byte> &input, uint32_t seed) const -> void {
    uint32_t seed2 = 0xEEEEEEEE;
...
auto Crypto::decryptTable(std::ifstream &file, const uint32_t size, const std::string_view name) const -> std::vector<uint32_t> {
