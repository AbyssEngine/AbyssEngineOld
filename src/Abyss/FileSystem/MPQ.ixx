...
class MPQStream : public SizeableStreambuf {
  ...
  protected:
    ...
    pos_type seekpos(pos_type pos, std::ios_base::openmode which) override { return seekoff(pos, std::ios_base::beg, which); }
    ...
    std::streamsize size() const override { return SFileGetFileSize(_mpqFile, nullptr); }
...
export class MPQ : public Provider {
  public:
    ...
    explicit MPQ(const std::filesystem::path &mpqPath) : _stormMpq(nullptr) {
    ...
    ~MPQ() { SFileCloseArchive(_stormMpq); }

    bool has(std::string_view fileName) override { return SFileHasFile(_stormMpq, fixPath(fileName).c_str()); }
    InputStream load(std::string_view fileName) override { return InputStream(std::make_unique<MPQStream>(_stormMpq, fixPath(fileName))); }
