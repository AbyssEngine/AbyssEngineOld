...
bool ImGui_ImplSDLRenderer2_Init(SDL_Renderer *renderer) {
    ...
    IM_ASSERT(io.BackendRendererUserData == nullptr && "Already initialized a _renderer backend!");
...
void ImGui_ImplSDLRenderer2_Shutdown() {
    ...
    IM_ASSERT(bd != nullptr && "No _renderer backend to shutdown, or already shutdown?");
    ImGuiIO &io = ImGui::GetIO();
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
            ...
            if (pcmd->UserCallback) {
                ...
                const SDL_Color *color =
                    (const SDL_Color *)(const void *)((const char *)(vtx_buffer + pcmd->VtxOffset) + IM_OFFSETOF(ImDrawVert, col)); // SDL 2.0.19+
#else
                const int *color =
                    (const int *)(const void *)((const char *)(vtx_buffer + pcmd->VtxOffset) + IM_OFFSETOF(ImDrawVert, col)); // SDL 2.0.17 and 2.0.18
#endif

                // Bind texture, Draw
                SDL_Texture *tex = (SDL_Texture *)pcmd->GetTexID();
                SDL_RenderGeometryRaw(bd->SDLRenderer, tex, xy, (int)sizeof(ImDrawVert), color, (int)sizeof(ImDrawVert), uv, (int)sizeof(ImDrawVert),
                                      cmd_list->VtxBuffer.Size - pcmd->VtxOffset, idx_buffer + pcmd->IdxOffset, pcmd->ElemCount, sizeof(ImDrawIdx));
            }
        }
    }
...
bool ImGui_ImplSDLRenderer2_CreateFontsTexture() {
    ...
    if (bd->FontTexture == nullptr) {
        SDL_Log("error creating _texture");
        return false;
    }
