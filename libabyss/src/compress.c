...
void *compress_decompress_wav(const void *buffer, uint32_t buffer_size, int channel_count, uint32_t *actual_read) {
   ...
   *actual_read = 0;
   ...
   streamreader_read_byte(input); // Skip a byte

   const uint8_t shift = streamreader_read_byte(input);


   for (int i = 0; i < channel_count; i++) {
       const int16_t temp = streamreader_read_int16(input);
       array_2[i] = (int)temp;
   ...
   while (!streamreader_eof(input)) {
       ...
       if (value & 0x80) {
           ...
           case 0:
               if (array_1[channel])
                   array_1[channel]--;
               streamwriter_push_int16(output, (int16_t)array_2[channel]);
               break;
           case 1:
               array_1[channel] += 8;
           ...
           streamwriter_push_int16(output, (int16_t)temp3);
   ...
   void *result = streamwriter_get_data(output, actual_read);
