...
void *compress_decompress_wav(void *buffer, uint32_t buffer_size, int channel_count) {
    ...
    while(!streamreader_eof(input)) {
        ...
        if (value & 0x80) {
            switch (value & 0x7f) {
                case 0:
                    if (array_1[channel]) array_1[channel]--;
                    streamwriter_push_int16(output, array_2[channel]);
                    break;
                case 1:
                    array_1[channel] += 8;
                    if (array_1[channel] > 0x58) array_1[channel] = 0x58;
                    if (channel_count == 2) channel = 1 - channel;
                    break;
                default:
                    array_1[channel] -= 8;
                    if (array_1[channel] < 0) array_1[channel] = 0;
                    if (channel_count == 2) channel = 1 - channel;
                    break;
            }
            continue;
        ...
        if (value & 1) temp2 += temp1 >> 0;
        if (value & 2) temp2 += temp1 >> 1;
        if (value & 4) temp2 += temp1 >> 2;
        if (value & 8) temp2 += temp1 >> 3;
        if (value & 0x10) temp2 += temp1 >> 4;
        if (value & 0x20) temp2 += temp1 >> 5;
        
        int temp3 = array_2[channel];
        if (value & 0x40)  {
            temp3 -= temp2;
            if (temp3 < -32768) temp3 = -32768;
        } else {
            temp3 += temp2;
            if (temp3 > 32767) temp3 = 32767;
        }
