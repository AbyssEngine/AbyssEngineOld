...
TtfFont::TtfFont(const std::filesystem::path &path, std::string_view name, int size, Cairo::FontOptions::HintStyle
        hint, Cairo::Antialias antialias)
...
std::unique_ptr<ITexture> TtfFont::RenderText(const std::string &text, int &width, int &height, eAlignment horizontalAlignment, int maxWidth) {
    ...
    Pango::Alignment align = Pango::Alignment::LEFT;
    ...
    Cairo::RefPtr<Cairo::ImageSurface> surface = Cairo::ImageSurface::create(Cairo::Surface::Format::ARGB32, width, height);
